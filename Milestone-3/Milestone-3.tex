...
\chapter{Design}
\label{sec:design}
This chapter details the design of the Project. The design of the Web Portal, API and Application are listed in the sections below. For each section the design process is described and the chosen designs outlines. The reasoning behind the chosen design are listed along with the reasoning for rejecting other possible designs. 
...
The Android Application has a detailed and intricate design which is described and detailed below. First the technology choices are outlined, followed by a section on rejected designs. Finally the final chosen design is detailed in depth and the choices are justified and explained.
...
Acceptance testing is the process of ensuring each of the specifications have been implemented. This in turn, confirms that each of the project's requirements have been met.

Acceptance testing was performed manually through the user interfaces of the web portal and Android application. Each set of acceptance tests, ensures that their respective set of specifications have been implemented. The acceptance criteria for each set of tests, specifies the behaviour of each specification that would consider them to be correct. Finally, a description of each set of test cases is given to provide an overview of the items that are being tested.
...
\begin{figure}[H]
   \centering
   \includegraphics[width=\textwidth]{CoocaRestClient} % requires the graphicx package
