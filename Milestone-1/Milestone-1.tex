...
The initial section of this document describes the purpose of the project, the client background and the project scope. It also includes a terminology section and a brief project overview. This is followed by an analysis of the team structure, including each team member?s role and responsibilities. A strategy for constant, effective communication is documented here. Section 3 discusses the product perspective and overview. It will also highlight the characteristics of its intended users, and constraints put upon the team by the client and a brief list of project assumptions. Section 4 focuses on the requirements set out by the client. It includes short, concise descriptions of functional, non-functional and nice to have requirements. Section 5 builds on the previous section, with each specification describing how the requirements will be achieved. Section 6 documents the cross-references, mapping each requirement to one or more specifications.

Section 7 highlights various software development methodologies that could be followed for this project. Each suitable methodology is illustrated and described in detail, along with advantages and disadvantages of using each. A brief sub-section highlights reasons why the chosen methodology was selected over the other available methodologies. A final sub-section discusses the various software testing techniques that will be used throughout the project. Section 8 documents a full project plan. It highlights the application and website backlogs, key project milestones and an overview of the project?s software development time table.

Section 9 focuses on the possible technical and personal risks associated with this particular project. Each risk is rated based on its likelihood and impact on the project and then placed into a risk matrix. All initial project work is documented in section 10. Here, minutes of meetings and user interface wire frames are presented. Future work will also be documented in this section.
...
This section briefly describes the product, its intended users, and the assumptions about the project so far. 
...
=======
>>>>>>> 6eb6c955efecc0e6f37312239e611b4555f7510a
\subsubsection{Spiral}
This section is a modified version of the work found in ``A RESTful Document Archival System''~\cite{restWeb}. The spiral methodology is a risk based system, which works iteratively towards a final working project. The process derives its name from the pattern with which its process follows. There are four basic activities that must occur in cycle loop of the spiral~\ref{fig:spiral}. These are:
...
By identifying risks at each cycle, the model allows developers to decide on the best approach for that cycle and minimise the chance of failure through correct mitigation of those risks. 
...
Generally Crystal clear is seen as good iterative-incremental process powered by planning and reviewing. It allows for continuous integration and is flexible and configurable to a users needs. However it only has limited scalability and does not share a common process with other crystal methods. It is also limited by its unsuitability for highly critical and over-dependence on inter-human communication. It also depends on a more day to day and continuous development period, which would make it difficult to apply to our project. 
